...
def main(args=sys.argv):
    ...
    parser.add_option("-p", "--port", action="callback", callback=lambda w, x, y, z: set_port(y),
                      default=cherrypy.config.get('server.socket_port'), type="int", help="Server port", metavar="PORT")
    parser.add_option("-b", "--blockstore", action="store", dest="blockstore", help="Path to the block store directory",
                      metavar="PATH", default=None)
    parser.add_option("-j", "--journaldir", action="store", dest="journaldir", help="Path to the job journal directory",
                      metavar="PATH", default=None)
    parser.add_option("-H", "--hostname", action="store", dest="hostname",
                      help="Hostname the master and other workers should use to contact this host", default=None)
    parser.add_option("-D", "--daemonise", action="store_true", dest="daemonise", help="Run as a daemon", default=False)
    parser.add_option("-o", "--logfile", action="store", dest="logfile", help="If daemonised, log to FILE",
                      default="/dev/null", metavar="FILE")
    parser.add_option("-v", "--verbose", action="callback",
                      callback=lambda w, x, y, z: ciel.set_log_level(logging.DEBUG), help="Turns on debugging output")
    parser.add_option("-6", "--task-log-root", action="store", dest="task_log_root",
                      help="Path to store task state log", metavar="PATH", default=None)
    parser.add_option("-i", "--pidfile", action="store", dest="pidfile", help="Record the PID of the process",
                      default=None);
